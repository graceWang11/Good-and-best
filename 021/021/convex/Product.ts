...
export const insertProducts = mutation({
  ...
  handler: async (ctx) => {
    const products = [
        ...
        { categoryname: "Accessories", brand: "Victor", series: "GR", productName: "TestProducts1", price: 999 },
        { categoryname: "Accessories", brand: "Victor", series: "GR", productName: "TestProducts2", price: 999 },
        { categoryname: "Accessories", brand: "Victor", series: "GR", productName: "TestProducts3", price: 999 },
        { categoryname: "Accessories", brand: "Victor", series: "GR", productName: "TestProducts4", price: 999 },
    ];
...
export const getAllProductsWithStock = query({
  handler: async (ctx) => {
    const products = await ctx.db.query("products").collect();
    const stocks = await ctx.db.query("stock").collect();

    return products.map(product => {
      const stockRecord = stocks.find(s => s.productID === product._id);
      return {
        ...product,
        stock: stockRecord?.stockQuantity || 0
      };
    });
  },
});
...
export const updateStock = mutation({
  ...
  handler: async (ctx, { productId, newStock }) => {
    const stockRecord = await ctx.db
      .query("stock")
      .filter(q => q.eq(q.field("productID"), productId))
      .first();
    ...
    if (stockRecord) {
      await ctx.db.patch(stockRecord._id, { stockQuantity: newStock });
    } else {
...
export const deleteProduct = mutation({
  ...
  handler: async (ctx, { productId }) => {
    ...
    if (stockRecord) {
      await ctx.db.delete(stockRecord._id);
    }
...
export const add = mutation({
  ...
  handler: async (ctx, args) => {
    // Get category ID
    const categoryRecord = await ctx.db
      .query("ProductCategory")
      .filter(q => q.eq(q.field("categoryName"), args.category))
      .first();
    ...
    const productId = await ctx.db.insert("products", {
      productCategoryID: categoryRecord._id,
      brand: args.brand ?? "",
      Series: args.series,
    ...
    // If it's shoes, add size information
    if (args.category === "Shoes" && args.size) {
      await ctx.db.insert("size", {
        productID: productId,
        SizeValue: args.size,
        SizeRegion: "UK", // Default to UK size
      });
    }
...
export const getCategories = query(async (ctx) => {
